...
let removeRequestContactSend = async (req, res) => {

  try {
    let currentUserId = req.user._id;
    let contactId = req.body.uid;

    let removeReq = await contact.removeRequestContactSend(currentUserId, contactId);
...
let readMoreContactsSend = async (req, res) => {
  try {
    ...
    // get more item
    let newContactUsers = await contact.readMoreContactsSend(req.user._id, skipNumberContacts);
...
let searchFriends = async (req, res) => {
  ...
  try {
    ...
    return res.render("main/groupChat/sections/_searchFriends", { users }); // render về view để append data
  } catch (error) {
    return res.status(500).send(error);
  }
};
