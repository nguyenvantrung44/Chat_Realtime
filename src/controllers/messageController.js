...
let addNewTextEmoji = async (req, res) => {
  ...
  try {
    ...
    let receiveId = req.body.uid;
    let messageVal = req.body.messageVal;
    let isChatGroup = req.body.isChatGroup;

    let newMessage = await message.addNewTextEmoji(sender, receiveId, messageVal, isChatGroup);
...
let storageImageChat = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    ...
    if (math.indexOf(file.mimetype) === -1) { // kiểm tra kiểu của file gửi lên có trong mảng math hay không
...
let addNewImage = async (req, res) => {
  ...
  imageMessageUploadFile(req, res, async (error) => {
    ...
    try {
      ...
      let newMessage = await message.addNewImage(sender, receiveId, messageVal, isChatGroup);

      // remove image khi upload vào mongodb
...
let addNewAttachment = async (req, res) => {
  ...
  attachmentMessageUploadFile(req, res, async (error) => {
    ...
    try {
      ...
      let newMessage = await message.addNewAttachment(sender, receiveId, messageVal, isChatGroup);

      // remove attachment khi upload vào mongodb
...
let readMoreAllChat = async (req, res) => {
  try {
    ...
    // get skipPersonal, skipGroup from query param
